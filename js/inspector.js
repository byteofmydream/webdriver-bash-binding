var SelectorLab = function () {
    ...
    var oldCss;
    var newCss = {backgroundColor: "#ffe", borderLeft: "5px solid #ccc"};
    ...
    function target_highlight() {
        ...
        if (typeof highlightedElement != "undefined") {
            if (typeof oldCss != "undefined") {
                $(highlightedElement).css(oldCss)
            }
        }
        highlightedElement = document.elementFromPoint(pos.left - window.pageXOffset - 1, pos.top - window.pageYOffset - 1);
        oldCss = $(highlightedElement).css(["backgroundColor", "borderLeft"]);
        $(highlightedElement).css(newCss);
